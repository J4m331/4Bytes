...
from App.controllers import (create_user, get_user, get_all_users,
                             get_all_users_json, jwt_required,
                             get_jwt_identity, createFile, getFile,
                             getAllFiles, deleteFile, getData, createData,
                             getFilebyName, createGraphData, getHeaders)
...
def download(file_id):
    ...
    return send_file(BytesIO(file.fileData),
...
def generateGraph(file_id, data_type):
    ...
    # If the request method is GET, render the page with the headers and file data
    if request.method == 'GET':
        files = getAllFiles()
        return render_template('index.html',
                               file_id=file_id,
    ...
    # If the data_type is provided, fetch and return the chart data as JSON
    if data_type:
        chart_data = createGraphData(file_id, data_type)
        return jsonify(chart_data)
    else:
        # Return an error if no valid data_type is provided
